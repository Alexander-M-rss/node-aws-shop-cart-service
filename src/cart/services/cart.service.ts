...
export class CartService {
  ...
  async findByUserId(userId: string): Promise<Cart>{
    ...
    .select('cart_items.product_id', 'cart_items.count', 'products.id', 'products.title', 'products.description', 'products.price')
  ...
  async createByUserId(userId: string): Promise<Cart>{
    ...
    return await pgClient('carts').insert(userCart).returning('*') as any as Cart;
  }
  ...
  async updateByUserId(userId: string, updateUserCartDTO: UpdateUserCartDTO): Promise<Cart>{
    const { id, items, ...rest } = await this.findOrCreateByUserId(userId) as any as Cart;
